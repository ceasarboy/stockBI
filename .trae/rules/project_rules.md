...
### 2026-02-24 实时行情与v1.0.0发布经验教训
1. **实时行情状态机**：使用状态机模式管理交易/非交易时间切换，避免频繁检查时间。
2. **增量数据更新**：首次全量加载分时数据，后续只获取最新一条数据追加，减少95%网络请求。
3. **板块代码规则**：创业板包括300和301开头，北交所包括920和8开头，需要在统计时正确分类。
4. **Pydantic模型**：FastAPI的POST请求参数使用`BaseModel`才能正确解析JSON body，普通类无法解析。
5. **Git大文件处理**：使用`git-filter-repo`清理历史中的大文件（如PostgreSQL数据），避免推送失败。
6. **Gitignore配置**：PostgreSQL数据文件(`macroquant/data/postgres/`)不应提交到仓库。
7. **async函数调用**：JavaScript中async函数调用后需要await等待完成，否则后续代码会在数据加载前执行。
8. **交易所判断逻辑**：根据股票代码开头判断交易所，需要在数据同步时设置exchange字段。
